...
- **toolMocks** for every write tool a scenario's jobs can reach, by `operationId`, with a string `result`. The check refuses a scenario that could hit a live write unmocked. Read tools, and login-style tools that carry `skipConfirmationReason` (phone lookup, PIN verification), may stay live, which is how the front door gets tested for real.
