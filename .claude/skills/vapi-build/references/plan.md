...
- **Tools** are the operations the agent may call. Include only what a job needs. The check fails on operations classified PRIVILEGED (admin, reset, internal) unless `agent.allowPrivileged` is true and the user asked for it. Any operation that writes, moves money, or deletes must have `confirmBeforeCall: true`; the compiled prompt then requires a read-back and an explicit yes.
...
- **Tests**: a handful of caller openings with observable expectations. Prefer scenarios drawn from transcripts, without copying a caller's personal details.
